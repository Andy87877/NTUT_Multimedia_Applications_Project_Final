# -*- coding: utf-8 -*-
"""
predict_vis_yolov4tiny_v2.py
=============================
Generates side-by-side visual comparison cards (Original | Ground Truth | V2 PyTorch Prediction)
specifically for the isolated Test Set images (split with seed 42) using the trained V2 weights.
"""
...
from train_pytorch_yolov4tiny_v2 import YoloV4Tiny, load_darknet_weights, ANCHORS, nms

# Paths
OBJ_DIR = PROJECT_ROOT / "obj"
WEIGHTS_PATH = PROJECT_ROOT / "jetbot_deploy" / "yolov4-tiny-416.weights"
OUT_DIR = PROJECT_ROOT / "runs" / "predict_vis_yolov4tiny_v2" / "test_results"
...
def draw_gt(img, label_path):
    """Draw ground-truth boxes (color-coded outline) onto img copy."""
    out = img.copy()
    h, w = out.shape[:2]
...
def draw_pred(img, detections):
    """Draw prediction boxes (solid colored top banner) onto img copy."""
    out = img.copy()
...
def make_card(img_path, label_path, detections, target_h=416):
    ...
    panel_pred = add_header(panel_pred, "YOLOv4-tiny V2", (30, 30, 140))
...
def main():
    print("\n" + "=" * 60)
    print("  YOLOv4-tiny V2 Test Set Prediction Visualizer")
    print("=" * 60)
    ...
    # 3. Get images and apply identical split
    ...
    print(f"[INFO] Found {len(test_imgs)} isolated Test Set images.")
    ...
    for idx, img_path in enumerate(test_imgs):
        ...
        # Decode detections (vectorized logic matching evaluation)
        detections = []
            ...
            for a in range(3):
                for y in range(H_out):
                    for x in range(W_out):
                        ...
                        # 0.3 threshold matching evaluation
                        if final_score > 0.3:
        ...
        if card is not None:
            out_path = OUT_DIR / f"vis_test_{img_path.stem}.jpg"
            ...
            print(f"  [{idx+1:2d}/{len(test_imgs)}] {img_path.name[:35]:<35} -> {cls_str}")

    # Build and save a single large grid image of all cards (3 columns)
    if all_cards:
        COLS = 3
        ...
        grid_path = PROJECT_ROOT / "runs" / "predict_vis_yolov4tiny_v2" / "test_grid_all.jpg"
        cv2.imwrite(str(grid_path), grid, [cv2.IMWRITE_JPEG_QUALITY, 85])
        print(f"\n[INFO] Test Set Grid saved -> {grid_path}")

    # Summary
    print()
    print("=" * 60)
    print("  YOLOv4-tiny V2 Test Set Inference Summary")
    print("=" * 60)
    print(f"  Processed images  : {stats['total']}")
    print(f"  Detected images   : {stats['detected']} ({stats['detected']/stats['total']*100:.1f}%)")
