...
def image_data_augmentation(mat, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp, gaussian_noise, blur,
    ...
    # Ensure output always matches target dimensions
    if sized.shape[0] != h or sized.shape[1] != w:
        sized = cv2.resize(sized, (w, h), interpolation=cv2.INTER_LINEAR)
...
class Yolo_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(use_mixup + 1):
            ...
            oh, ow, oc = img.shape
            dh, dw, dc = np.array(
                np.array([oh, ow, oc]) * self.cfg.jitter, dtype=np.int32)
    ...
    def _get_val_item(self, index):
        ...
        bboxes_with_cls_id = np.array(
            self.truth.get(img_path), dtype=np.float64)
