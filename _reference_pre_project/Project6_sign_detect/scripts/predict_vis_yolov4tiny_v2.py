...
from _reference_pre_project.Project6_sign_detect.scripts.train_pytorch_yolov4tiny_v2 import YoloV4Tiny, load_darknet_weights, ANCHORS, nms
import sys
import os
import random
...
def main():
    ...
    for idx, img_path in enumerate(test_imgs):
        ...
        if card is not None:
            ...
            print(
                f"  [{idx+1:2d}/{len(test_imgs)}] {img_path.name[:35]:<35} -> {cls_str}")
    ...
    if all_cards:
        ...
        grid_path = PROJECT_ROOT / "runs" / \
            "predict_vis_yolov4tiny_v2" / "test_grid_all.jpg"
