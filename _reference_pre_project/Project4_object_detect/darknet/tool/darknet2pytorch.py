...
import numpy as np
from _reference_pre_project.Project4_object_detect.darknet.tool.region_loss import RegionLoss
from _reference_pre_project.Project4_object_detect.darknet.tool.yolo_layer import YoloLayer
from _reference_pre_project.Project4_object_detect.darknet.tool.config import *
        ...
        for block in blocks:
            if block['type'] == 'net':
                ...
                if activation == 'leaky':
                    ...
                    model.add_module('linear{0}'.format(
                        conv_id), nn.Identity())
                elif activation == 'logistic':
                    model.add_module(
                        'sigmoid{0}'.format(conv_id), nn.Sigmoid())
                else:
