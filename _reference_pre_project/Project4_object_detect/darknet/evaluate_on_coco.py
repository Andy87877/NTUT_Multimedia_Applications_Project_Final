...
from _reference_pre_project.Project4_object_detect.darknet.tool.utils import load_class_names
...
def test(model, annotations, cfg):
    ...
    throwaway_image = Image.open(
        'data/dog.jpg').convert('RGB').resize((model.width, model.height))
    ...
    for i, image_annotation in enumerate(images):
        ...
        img = Image.open(os.path.join(
            cfg.dataset_dir, image_file_name)).convert('RGB')
