...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        loss_x = self.coord_scale * \
            nn.MSELoss(reduction='sum')(x * coord_mask, tx * coord_mask) / 2.0
        loss_y = self.coord_scale * \
            nn.MSELoss(reduction='sum')(y * coord_mask, ty * coord_mask) / 2.0
        loss_w = self.coord_scale * \
            nn.MSELoss(reduction='sum')(w * coord_mask, tw * coord_mask) / 2.0
        loss_h = self.coord_scale * \
            nn.MSELoss(reduction='sum')(h * coord_mask, th * coord_mask) / 2.0
        loss_conf = nn.MSELoss(reduction='sum')(
            conf * conf_mask, tconf * conf_mask) / 2.0
        loss_cls = self.class_scale * \
            nn.CrossEntropyLoss(reduction='sum')(cls, tcls)
        ...
        print('%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f' % (
            self.seen, nGT, nCorrect, nProposals, loss_x.data[
                0], loss_y.data[0], loss_w.data[0], loss_h.data[0],
