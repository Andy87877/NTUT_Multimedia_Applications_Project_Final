...
| :---: | :---: | :--- |
| 0 | ![Blocked](images/sign_detection/blocked.png){width=0.48in} | 在路牌前停止並進入鎖定狀態，不自行恢復行駛 |
| 1 | ![Pedestrian](images/sign_detection/pedestrian.png){width=0.48in} | 將速度降低為原設定的 0.7 倍，並保持減速 1 秒 |
| 2 | ![Rail](images/sign_detection/rail.png){width=0.48in} | 原地停止 5 秒，再等待新影格後恢復道路跟隨 |
| 3 | ![Stop](images/sign_detection/stop.png){width=0.48in} | 原地停止 2 秒，再等待新影格後恢復道路跟隨 |
...
Project 6 的 YOLOv4-tiny 完成 1000 Epochs 訓練，驗證集 F1-Score 為 93.30%。在 15 張隔離測試影像、34 個路牌目標及信心度門檻 0.80 下，整體 F1-Score 約為 97.06%。類別順序已確認為 `0=blocked`、`1=pedestrian`、`2=rail`、`3=stop`，與 `Final_team_1.ipynb` 的控制動作一致。
