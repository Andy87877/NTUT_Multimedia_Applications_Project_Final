...
from _reference_pre_project.Project4_object_detect.darknet.tool.yolo_layer import YoloLayer
    ...
    def forward(self, x, target_size, inference=False):
        ...
        if inference:
            ...
            return x.view(x.size(0), x.size(1), x.size(2), 1, x.size(3), 1).\
                ...
                contiguous().view(x.size(0), x.size(
                    1), target_size[2], target_size[3])
        else:
...
class Yolov4(nn.Module):
    def __init__(self, yolov4conv137weight=None, n_classes=80, inference=False):
        ...
        if yolov4conv137weight:
            ...
            pretrained_dict = {k1: v for (k, v), k1 in zip(
                pretrained_dict.items(), model_dict)}
...
if __name__ == "__main__":
    ...
    model = Yolov4(yolov4conv137weight=None,
                   n_classes=n_classes, inference=True)
    ...
    from _reference_pre_project.Project4_object_detect.darknet.tool.utils import load_class_names, plot_boxes_cv2
    from _reference_pre_project.Project4_object_detect.darknet.tool.torch_utils import do_detect
