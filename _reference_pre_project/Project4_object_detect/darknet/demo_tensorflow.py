...
import cv2
from _reference_pre_project.Project4_object_detect.darknet.tool.utils import post_processing, load_class_names, plot_boxes_cv2
...
def demo_tensorflow(tfpb_file="./weight/yolov4.pb", image_path=None, print_sensor_name=False):
    ...
    with tf.compat.v1.Session() as persisted_sess:
        ...
        resized = cv2.resize(
            image_src, (inp.shape[2], inp.shape[3]), interpolation=cv2.INTER_LINEAR)
        ...
        result = plot_boxes_cv2(
            image_src, boxes, savename=None, class_names=class_names)
